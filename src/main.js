import { testAddSuccessListener } from "./utils/testScripts.js"
...
window._confirmationBoosterData.senderEmailAddress =
  window._confirmationBoosterSettings.senderEmailAddress
window._confirmationBoosterData.imagesPath =
  window._confirmationBoosterSettings.imagesPath
...
function generateButton(options) {
  return `
    ...
    <img alt='' src="${window._confirmationBoosterData.imagesPath}${options.iconFilename}" height="20" style="height: 20px;" />
...
const buttonsHTML = (options) => {
  ...
  if (options.provider) {
    finalHTML += generateButton({
      ...
      iconFilename: options.provider.iconFilename,
    })
  }
